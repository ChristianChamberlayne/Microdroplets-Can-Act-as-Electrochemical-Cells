...
function [U,storePath,RadialSolutions] = Solve_ion_to_nuetral(TargetN_kions, TargetDiffPS_kions, TargetS_kions, R, guessPoint,PointsInDerivative,manyGraphs,verbose)
...
PointsInDerivative = PointsInDerivative;   % later, make sure this matches number of points in TargetS
DerivativeStepRange = .01; %change this back to .001 maybe if this doesnt work out
showSolutionGraph = manyGraphs;  % this is for trouble shooting midfit function, turn off to speed up solve. 



TargetN = TargetN_kions; % in kions
TargetDiffPS = TargetDiffPS_kions; % in kions


TargetS = TargetS_kions * linspace(1 - DerivativeStepRange,1 + DerivativeStepRange,PointsInDerivative); 
TargetP = TargetDiffPS + TargetS;
TargetNS = TargetN + TargetS;
...
%Check inputs are good
if TargetN_kions < 0
    disp('Impossible targets. \n negative number of N ions \aborting solve of this point')
     U = [NaN, NaN;...
          NaN, NaN;...
          NaN, NaN];
     storePath = NaN;
...
if any(TargetP(:) < 0)
    disp('Impossible targets. \n negative number of P ions \aborting solve of this point')
     U = [NaN, NaN;...
          NaN, NaN;...
          NaN, NaN];
     storePath = NaN;
...
if any(TargetS(:) < 0)
    disp('Impossible targets. \n negative number of S ions \aborting solve of this point')
     U = [NaN, NaN;...
          NaN, NaN;...
          NaN, NaN];
     storePath = NaN;
...
for i = 1:2
    if verbose, fprintf('%i ',i), end
        ...
        RatioNeg = -(TargetNS(1) - TargetS(1)) / (TargetS(1) - TargetP(1) * (1/PathRatioPos(i) + 1) + TargetNS(1) - TargetS(1));
        ...
        [Sxint, xint, sucessfulSolve,rGuess] = midfit_PTarget_V3(TargetP(1),PathRatioPos(i),RatioNeg,DropParam,innerConc,rGuess,showSolutionGraph);
...
if (j ~= 1)  %skip getting points from previous loop solution if first loop 
    ...
    for i = 1:2
        ...
        %calculate the function for RatioNeg dependence on RatioPos that would result with given targets.
        %this equation enforces that the net charge on the microdroplet is 0. Modify for charged droplets 
        RatioNeg = -(TargetNS(j) - TargetS(j)) / (TargetS(j) - TargetP(j) * (1/PathRatioPos(i) + 1) + TargetNS(j) - TargetS(j));
...
for i = 3:maxloops
    ...
    pointExtrapolation = (TargetNS(j) - MolesNegativeIons(i-1))*(PathRatioPos(i-2) - PathRatioPos(i-1))/(MolesNegativeIons(i-2) - MolesNegativeIons(i-1)) + PathRatioPos(i-1);
    ...
    for k = 1:maxSolveTries   %break out of this loop if solve is sucessfull. 
        ...
        %calculate point
        [Sxint, xint, sucessfulSolve,rGuess] = midfit_PTarget_V3(TargetP(j),PathRatioPos(i),RatioNeg,DropParam,innerConc,rGuess,showSolutionGraph);
        ...
        if(k == maxSolveTries) 
            fprintf('WARNING: k hit max loops, will end solve of this point. Location j,i: (%i,%i) \n in function for TargetP(j) %g, TargetNS_kions %g, TargetS_kions %g, R %g',...
                    j,i,TargetP(j), TargetNS_kions, TargetS_kions, R) 
            U = [NaN, NaN;...
                 NaN, NaN;...
                 NaN, NaN];
            storePath = NaN;
    ...
    if abs(MolesNegativeIons(i) - TargetNS(j)) < precisionNS 
%&& abs(MolesPositiveIons(i) - TargetP(j)) < precisionP) || i == maxloops
    ...
    if manyGraphs
        ...
        title(sprintf('solve path for j = %i \n targets P NS and S are: %.2e %.2e %.2e', j, TargetP(j), TargetNS(j), TargetS(j)))
        drawnow()
        ...
        figure(103)
        plot(1:i,MolesNegativeIons(1:i)/TargetNS(j))
        title(sprintf('solve path for j = %i', j))
        title(sprintf('solve path for j = %i \n targets NS is: %.2e \n ratio NS/targetNS ', j, TargetNS(j)))
        drawnow()
        
        figure(104)
        plot(PathRatioPos,MolesNegativeIons(1:i)/TargetNS(j),'-o')
        title(sprintf('solved curve for j = %i \n targets NS is: %.2e \n ratio NS/targetNS ', j, TargetNS(j)))
        drawnow()
    end 
end
    %stuff from each j loops that get returned
    storePath{j} = [PathRatioPos; TargetP(j)*ones(1,i); MolesNegativeIons(1:i); MolesPositiveIons(1:i); MolesSurfaceIons(1:i)];
