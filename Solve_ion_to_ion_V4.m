...
function [U,storePath,RadialSolutions] = Solve1D_V4(TargetP_kions, TargetNS_kions, TargetS_kions, R, guessPoint,PointsInDerivative,DerivativeStepRange, manyGraphs,verbose)
...
for i = 3:maxloops
    ...
    for k = 1:maxSolveTries   %break out of this loop if solve is sucessfull. 
        ...
        else %ie bad solution, take smaller step (currently 1/2th of previous step)
                if verbose,fprintf('(k = %i) ',k),end
                pointExtrapolation = .5 * (pointExtrapolation(1) - PathRatioPos(i-1)) + PathRatioPos(i-1);
                rGuess = 48/50 * R; %reset rGuess 
        end
        
        if(k == maxSolveTries) 
            fprintf('WARNING: k hit max loops, will continue solve. Location j,i: (%i,%i) \n in function for TargetP_kions %g, TargetNS_kions %g, TargetS_kions %g, R %g',...
